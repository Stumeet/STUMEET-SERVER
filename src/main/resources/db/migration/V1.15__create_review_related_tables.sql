...
(
    ...
    `rate`        TINYINT       NOT NULL COMMENT '평점',
...
(
    `id`       BIGINT       NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT '리뷰 태그 ID',
...
(
    ...
    `review_tag_id` BIGINT NOT NULL COMMENT '리뷰 태그 ID',
    CONSTRAINT `fk_review_tag_usage_review_id` FOREIGN KEY (`review_id`)
        REFERENCES `review` (`id`)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk_review_tag_usage_review_tag_id` FOREIGN KEY (`review_tag_id`)
        REFERENCES `review_tag` (`id`)
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;